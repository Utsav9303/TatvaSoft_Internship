...
namespace Mission.Repositories.Repositories
{
    public class MissionRepository(MissionDbContext dbContext) : IMissionRepository
    {
        ...
        public async Task<bool> DeleteMissionApplication(int id)
        {
            try
            {
                var application = await _dbContext.MissionApplications.FindAsync(id);
                if (application == null)
                    throw new Exception("Mission application not found");

                var mission = await _dbContext.Missions.FindAsync(application.MissionId);
                if (mission != null)
                {
        ...
        public async Task<bool> UpdateMission(Missions model)
        {
            try
            {
                var existingMission = await _dbContext.Missions.FindAsync(model.Id);
                if (existingMission == null)
                    throw new Exception("Mission not found");

                existingMission.MissionTitle = model.MissionTitle;
                existingMission.MissionDescription = model.MissionDescription;
                existingMission.MissionImages = model.MissionImages;
                existingMission.StartDate = model.StartDate;
                existingMission.EndDate = model.EndDate;
                existingMission.CountryId = model.CountryId;
                existingMission.CityId = model.CityId;
                existingMission.TotalSheets = model.TotalSheets;
                existingMission.MissionThemeId = model.MissionThemeId;
                existingMission.MissionSkillId = model.MissionSkillId;
