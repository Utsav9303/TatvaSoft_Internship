            ...
            try
            {
                if (files != null && files.Count > 0)
                {
                    foreach (var file in files)
                    {
                        ...
                        var fName = string.Join("", file.FileName.Split(Path.GetInvalidFileNameChars()));
                        fName = fName.Replace(" ", "");
                        
                        var fPath = Path.Combine("UploadedImage", "Mission");
                        string fileRootPath = Path.Combine(Directory.GetCurrentDirectory(), "UploadedImage", "Mission");
                        ...
                        if (System.IO.File.Exists(fullRootPath))
                        {
                            System.IO.File.Delete(fullRootPath);
                        }
