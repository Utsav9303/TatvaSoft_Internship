...
namespace Mission.Controllers
{
    ...
    public class MissionController : ControllerBase
    {
        ...
        public async Task<IActionResult> MissionApplicationDelete([FromBody] int id)
        {
            try
            {
                var result = await _missionService.DeleteMissionApplication(id);
                return Ok(new ResponseResult() { Data = result, Message = "Mission application deleted successfully", Result = ResponseStatus.Success });
            }
            catch (Exception ex)
            {
                return BadRequest(new ResponseResult() { Data = null, Message = ex.Message, Result = ResponseStatus.Error });
