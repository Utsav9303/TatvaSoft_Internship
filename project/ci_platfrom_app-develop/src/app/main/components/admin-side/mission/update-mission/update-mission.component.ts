...
export class UpdateMissionComponent implements OnInit, OnDestroy {
  ...
  getMissionSkillList() {
    const getMissionSkillListSubscription = this._service.getMissionSkillList().subscribe(
      (data: any) => {
        if (data.result == 1) {
          this.missionSkillList = data.data.map((skill: any) => ({
            value: skill.missionSkillId,
            text: skill.skillName
          }));
          console.log('Mission Skills:', this.missionSkillList);
        } else {
          this._toast.error({ detail: "ERROR", summary: data.message, duration: APP_CONFIG.toastDuration });
        }
      },
    ...
    const getMissionThemeListSubscription = this._service.getMissionThemeList().subscribe({
      next: (data: any) => {
        ...
        if (data.result === 1 && data.data && data.data.length > 0) {
          ...
          console.log('Processed mission themes:', this.missionThemeList);
        } else {
          console.error('No active mission themes found:', data);
          this._toast.error({ 
            detail: "ERROR", 
            summary: "No active mission themes found. Please add some mission themes first.", 
            duration: APP_CONFIG.toastDuration 
          });
        }
      },
      error: (err) => {
        console.error('Error fetching mission themes:', err);
        this._toast.error({ 
          detail: "ERROR", 
          summary: err.message || 'Failed to fetch mission themes', 
          duration: APP_CONFIG.toastDuration 
        });
      }
    });
